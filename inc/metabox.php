...
function mountaviary_project_metabox($post)
{
    ...
    ?>
    <div>
        <input type="url" name="project-link" class="widefat" placeholder="Enter Link"
...
function mountaviary_services_metabox($post)
{
    ...
    ?>
        <div>
            <input type="text" name="service-icon" class="widefat" placeholder="Put your icon name"
                value="<?php echo get_post_meta($post->ID, 'service-icon', true); ?>">
            <label for="service-icon">To add your favourite icon, <a target="_blank"
                    href="https://developer.wordpress.org/resource/dashicons/">Follow this Link</a> and copy the icon name
                clicking on it. and paste the name here. For Example: <i>dashicons-index-card</i></label>
        </div>
        <?php
}
