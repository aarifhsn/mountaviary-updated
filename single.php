...
<section id="blog" class="blog_posts single_page min-h-[100vh] max-w-screen-md mx-auto my-4 font-poppins  px-4">
  <div class="blog_info_area dark:text-slate-200 ">
    <?php if (have_posts()): ?>

      <?php
      while (have_posts()):
        the_post(); ?>
        <div <?php post_class('single_blog bg-white dark:bg-slate-900 dark:text-slate-100 shadow-sm mb-4 rounded-lg leading-loose'); ?>>
          <?php
          if (is_singular()):
          ...
          <div class="blog_content px-4 py-3">
            ...
            <div class="author_info flex items-center">
              <h4
                class="author_name text-slate-500 dark:text-slate-400 hover:text-slate-600 dark:hover:text-slate-500 my-3 mr-2 text-xs font-bold">
                <?php the_author_posts_link(); ?>
              </h4>
              <h5 class="post_date text-slate-500 dark:text-slate-400 text-xs">
                <?php the_date('M d, Y'); ?>
              </h5>
              <div>
                  ...
                  if (!empty($categories)) {
                    foreach ($categories as $index => $category) {
                      ...
                      echo '<li class="category-item category-' . esc_attr($index) . '">
                                        <a class="text-sm bg-slate-100 dark:bg-slate-950 p-1 rounded text-slate-700 dark:text-slate-200  hover:text-slate-900 hover:bg-slate-200 " href="' . esc_url($category_link) . '">' . esc_html($category->name) . '</a>
                                      </li>';
                    }
                  }
                  ?>
            ...
            <div class="single_content text-slate-800 dark:text-slate-300 mb-2 overflow-hidden">
              <?php the_content(); ?>
            </div>
          </div>
        </div>
