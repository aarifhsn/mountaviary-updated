		...
		<div class="mb-8 pb-6 border-b border-slate-100 dark:border-slate-800">
			...
			<?php if (have_posts()): ?>
				<p class="text-sm text-slate-400 dark:text-slate-500 mt-1">
					<?php printf(
						esc_html(_n('%s result found', '%s results found', $wp_query->found_posts, 'mountaviary')),
		...
		<?php if (have_posts()): ?>
			...
			<div class="space-y-0">
				<?php while (have_posts()):
					...
					<article <?php post_class('group flex gap-4 py-5 border-b border-slate-100 dark:border-slate-800 last:border-0'); ?>>
		...
		<?php else: ?>
			...
			<div class="text-center py-12">
				...
				<p class="text-base font-semibold text-slate-700 dark:text-slate-300 mb-1">
					<?php esc_html_e('No results found', 'mountaviary'); ?>
				</p>
				<p class="text-sm text-slate-400 dark:text-slate-500 mb-8">
					<?php esc_html_e('Try different keywords or browse the site below.', 'mountaviary'); ?>
				</p>
				<?php get_search_form(); ?>
			</div>
